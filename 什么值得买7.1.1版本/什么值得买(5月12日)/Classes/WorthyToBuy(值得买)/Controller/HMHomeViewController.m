...
/** collectionView数据源 */
@property (nonatomic, strong) NSArray <HMHomeChannel *> *dataArray;
...
- (void)setUpScrollView{
    ...
    [self.dataArray enumerateObjectsUsingBlock:^(HMHomeChannel * _Nonnull channel, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    HMHomeCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"HMHomeCell" forIndexPath:indexPath];
    
    //防止cell.contentView重复添加控制器的View
//    [cell.contentController.view removeFromSuperview];
    
    HMHomeChannel *channel = self.dataArray[indexPath.item];
    HMContentViewController *contentVc = [self controllerWithChannel:channel];
...
- (HMContentViewController *)controllerWithChannel:(HMHomeChannel *)channel{
    
    
    //先判断缓存中有没有, 没有的话就自己创建
    HMContentViewController *contentController = [self.controllerCache objectForKey:channel.title];
    
    if (!contentController) {
        contentController = [[UIStoryboard storyboardWithName:@"HMContentViewController" bundle:nil] instantiateInitialViewController];
...
- (NSArray <HMHomeChannel *> *)dataArray {
	if(_dataArray == nil) {
        _dataArray = [HMHomeChannel homeChannels];
	}
	return _dataArray;
}
