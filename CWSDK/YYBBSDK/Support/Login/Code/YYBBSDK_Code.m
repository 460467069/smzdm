...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    self.confirmAction.enabled = textField.text.length >= YYBBCodeLength;
}
...
- (void)textFieldDidChange:(UITextField *) textField {
    ...
    if (length > 0) {
        if (length > YYBBCodeLength) {
            textField.text = [textField.text substringToIndex:YYBBCodeLength];
        }
    }
    self.confirmAction.enabled = length >= YYBBCodeLength;
}
